...
from tpnsta.data.loader import data_loader, data_loader_TPN
from tpnsta.losses import gan_g_loss, gan_d_loss, l2_loss
from tpnsta.losses import displacement_error, final_displacement_error

from tpnsta.models import TrajectoryGenerator, TrajectoryDiscriminator, TrajectoryGeneratorMultiMergeSlinearAveF
from tpnsta.utils import int_tuple, bool_flag, get_total_norm
...
# Optimization
parser.add_argument('--batch_size', default=64, type=int)  # default 64
...
# Generator Options
parser.add_argument('--encoder_h_dim_g', default=32, type=int)  # FIXME default 32,
...
# Pooling Options
parser.add_argument('--pooling_type', default='st_attention')
...
parser.add_argument('--d_learning_rate', default=2e-4, type=float)
parser.add_argument('--d_steps', default=1, type=int)
parser.add_argument('--clipping_threshold_d', default=0, type=float)

# Loss Options
parser.add_argument('--l2_loss_weight', default=1.0, type=float)
parser.add_argument('--angle_loss_weight', default=0.0, type=float)  # FIXME loss weight hyper-parameter
...
parser.add_argument('--checkpoint_name', default='TPNSTA_ETH')
...
parser.add_argument('--seed', default=559, type=int)  # FIXME zara1 2003
...
def main(args):
    ...
    iterations_per_epoch = len(train_dset)/args.batch_size/(args.d_steps + args.g_steps)   # generator and discriminator ,we need to /2.0
    if args.num_epochs:
    ...
    while t < args.num_iterations:
        ...
        for batches in train_loader:
            ...
            if d_steps_left > 0:
                # scheduler_d.step(epoch)  # change the way to optimize
                step_type = 'd'
                ...
                losses_g = generator_step(args, batch_4, batch_8, batch_16, batch_32, generator,
                                          ...
                                          traj_std_y_8)     # default is optimizer_g
            ...
            if t > 0 and t % int(iterations_per_epoch * 40) == 0:
                ...
                if metrics_val['ade'] == min_ade:
                    ...
                    # checkpoint['merge_best_state'] = merge_model.state_dict()
...
def generator_step(
    ...
    g_l2_loss_rel_32 = []
    g_l2_loss_rel_final = []
    loss_weight = [2.0, 1.0, 0.5, 0.25, 0.25, 2]  # FIXME weights of multiscale output or [2.0,1.0,0.5,0.25,1.0]
...
def check_accuracy(
    ...
    with torch.no_grad():
        for batches in loader:
            ...
            # pred_traj_fake = relative_to_abs(unstandar_pred_traj_fake_rel, obs_traj[-1])
