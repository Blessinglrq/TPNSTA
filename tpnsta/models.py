...
from tpnsta.utils import relative_to_abs
...
def get_noise_multi(shape, noise_type, var):
    if noise_type == 'gaussian':
        return torch.randn(*shape).add_(var).cuda()
    elif noise_type == 'uniform':
        return torch.rand(*shape).sub_(0.5).mul_(2.0).cuda()
    raise ValueError('Unrecognized noise type "%s"' % noise_type)
...
class Decoder(nn.Module):
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end, seq_len):
        """
        Inputs:
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, seq_len, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        ...
        # Encode seq
        final_encoder_h = self.encoder(obs_traj_rel)  # (self.num_layers, batch, self.h_dim)
        # Pool States
        if self.pooling_type:
            end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net(final_encoder_h, seq_start_end, end_pos)   # (batch,pool_dim)
            # Construct input hidden states for decoder
            mlp_decoder_context_input = torch.cat(
                [final_encoder_h[-1].view(-1, self.encoder_h_dim), pool_h], dim=1)
        else:
            mlp_decoder_context_input = final_encoder_h[-1].view(
                -1, self.encoder_h_dim)  # FIXME final_encoder_h --> final_encoder_h[-1]
        ...
        decoder_h = self.add_noise(
            noise_input, seq_start_end, user_noise=user_noise)  # 将隐藏状态加上噪声向量
        decoder_h = torch.unsqueeze(decoder_h, 0)
...
class MergeModelConv(torch.nn.Module):
    def __init__(self):
        ...
        #     torch.nn.Conv2d(4, 8, kernel_size=1, stride=1, bias=True),
        #     torch.nn.LeakyReLU(),
        #     # torch.nn.Conv2d(8, 16, kernel_size=1, stride=1, bias=False),
        #     # torch.nn.LeakyReLU(),
        #     # torch.nn.Conv2d(16, 8, kernel_size=1, stride=1, bias=False),
        #     # torch.nn.LeakyReLU(),
        #     torch.nn.Conv2d(8, 4, kernel_size=1, stride=1, bias=True),
        #     torch.nn.LeakyReLU(),
        #     torch.nn.Conv2d(4, 1, kernel_size=1, stride=1, bias=True),
        #     torch.nn.LeakyReLU(),
        # )
...
class TrajectoryGeneratorMultiMergeSlinearAveF(nn.Module):
    def __init__(
        self, obs_len, embedding_dim=64, encoder_h_dim=64,
        decoder_h_dim=128, mlp_dim=1024, num_layers=1, noise_dim=(0, ),
        noise_type='gaussian', noise_mix_type='ped', pooling_type=None,
        ...
        activation='leakyrelu', batch_norm=True, neighborhood_size=2.0, grid_size=8
            ...
            super(TrajectoryGeneratorMultiMergeSlinearAveF, self).__init__()
            ...
            if pooling_type:
                self.tanh = nn.Tanh()
                # self attention's weight
                self.wt1 = nn.Parameter(torch.randn(self.da, self.decoder_h_dim))
                self.wt2 = nn.Parameter(torch.randn(1, self.da))
                self.bias_t = nn.Parameter(torch.randn(self.da, 1))
    ...
    def temporal_attentionnew(self, hidden_states):
        hidden_states = hidden_states.permute(1, 0, 2)  # (batch,T,32)
        alpha_1 = self.tanh(torch.matmul(self.wt1, hidden_states.permute(0, 2, 1)) + self.bias_t)
        alpha = F.softmax(torch.matmul(self.wt2, alpha_1), dim=2)
        hidden_states = torch.matmul(alpha, hidden_states)[:, 0]  # (batch, hidden)
        return hidden_states

    def spatio_attention_norm(self, hidden_states, rel_pos):
        rel_pos = (rel_pos[:, :, 0].pow(2) + rel_pos[:, :, 1].pow(2)).sqrt()
        norm = rel_pos.pow(2).sum(1).sqrt().unsqueeze(1)
        norm_p = rel_pos/norm
        S = torch.matmul(norm_p, norm_p.permute(1, 0))  # (num_ped, num_ped)
        beta = F.softmax((S), dim=1)
        hidden_states = torch.matmul(beta, hidden_states)   # (num_ped, hidden)
        return hidden_states

    def SpatioTemporalSelfAttentionnew(self, h_states, seq_start_end, end_pos):
        pool_h = []
        # Temporal Attention mechanism
        h_states = self.temporal_attentionnew(h_states)  # (T, batch, hidden) --->(batch, hidden)
        for _, (start, end) in enumerate(seq_start_end):
            start = start.item()
            end = end.item()
            num_ped = end - start
            curr_hidden = h_states.view(-1, self.decoder_h_dim)[start:end]

            curr_end_pos = end_pos[start:end]
            curr_end_pos_1 = curr_end_pos.repeat(num_ped, 1)
            curr_end_pos_2 = self.repeat(curr_end_pos, num_ped)
            curr_rel_pos = curr_end_pos_2 - curr_end_pos_1  # 得到行人的end_pos间的相对关系，并交给感知机去具体处理。每个行人与其他行人的相对位置关系由num_ped项，合计有num_ped**2项。

            # Spatio Attention mechanism
            curr_rel_pos = curr_rel_pos.view(-1, num_ped, 2)  # (num_ped, neighbor, hidden)
            curr_hidden = self.spatio_attention_norm(curr_hidden, curr_rel_pos)
            pool_h.append(curr_hidden)
    ...
    def DecoderMulti(self, hidden_state, last_pos, last_pos_rel, state_tuple, seq_start_end, seq_len):
        batch = last_pos.size(0)
        pred_traj_fake_rel = []
        decoder_input = self.spatial_embedding(last_pos_rel)
        decoder_input = decoder_input.view(1, batch, self.embedding_dim)
        ...
        for _ in range(seq_len):
            ...
            if self.pool_every_timestep:
                ...
                pool_h = self.SpatioTemporalSelfAttentionnew(hidden_state, seq_start_end, curr_pos)
                decoder_h = torch.cat(
    ...
    def forward(self, obs_traj_4, obs_traj_rel_4, obs_traj_8, obs_traj_rel_8, obs_traj_16, obs_traj_rel_16, obs_traj_32, obs_traj_rel_32, seq_start_end, seq_len, user_noise=None):
        ...
        if self.pooling_type:
            ...
            final_attention_encoder_h_4 = self.SpatioTemporalSelfAttentionnew(final_encoder_h_4, seq_start_end, end_pos_4)
            final_attention_encoder_h_8 = self.SpatioTemporalSelfAttentionnew(final_encoder_h_8, seq_start_end, end_pos_8)
            final_attention_encoder_h_16 = self.SpatioTemporalSelfAttentionnew(final_encoder_h_16, seq_start_end, end_pos_16)
            final_attention_encoder_h_32 = self.SpatioTemporalSelfAttentionnew(final_encoder_h_32, seq_start_end, end_pos_32)
            # Construct input hidden states for decoder
            mlp_decoder_context_input_4 = torch.cat(
                [final_encoder_h_4[-1].view(-1, self.encoder_h_dim), final_attention_encoder_h_4], dim=1)
            mlp_decoder_context_input_8 = torch.cat(
                [final_encoder_h_8[-1].view(-1, self.encoder_h_dim), final_attention_encoder_h_8], dim=1)
            mlp_decoder_context_input_16 = torch.cat(
                [final_encoder_h_16[-1].view(-1, self.encoder_h_dim), final_attention_encoder_h_16], dim=1)
            mlp_decoder_context_input_32 = torch.cat(
                [final_encoder_h_32[-1].view(-1, self.encoder_h_dim), final_attention_encoder_h_32], dim=1)
        else:
            mlp_decoder_context_input_4 = final_encoder_h_4[-1].view(
                -1, self.encoder_h_dim)
            mlp_decoder_context_input_8 = final_encoder_h_8[-1].view(
                -1, self.encoder_h_dim)
            mlp_decoder_context_input_16 = final_encoder_h_16[-1].view(
                -1, self.encoder_h_dim)
            mlp_decoder_context_input_32 = final_encoder_h_32[-1].view(
                -1, self.encoder_h_dim)
        ...
        decoder_h_4 = self.add_noise(
            noise_input_4, seq_start_end, user_noise=user_noise)
        decoder_h_4 = decoder_h_4.repeat(self.num_layers, 1, 1)

        decoder_h_8 = self.add_noise(
            noise_input_8, seq_start_end, user_noise=user_noise)

        decoder_h_8 = decoder_h_8.repeat(self.num_layers, 1, 1)
        decoder_h_16 = self.add_noise(
            noise_input_16, seq_start_end, user_noise=user_noise)

        decoder_h_16 = decoder_h_16.repeat(self.num_layers, 1, 1)
        decoder_h_32 = self.add_noise(
            noise_input_32, seq_start_end, user_noise=user_noise)

        decoder_h_32 = decoder_h_32.repeat(self.num_layers, 1, 1)
        ...
        rel_curr_ped_seq_4_m = rel_curr_ped_seq_4_o.unsqueeze(0)
        ...
        # FIXME merge result for conv  model 2 / model 4
